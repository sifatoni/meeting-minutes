...
def main():
    ...
    transcript = transcribe_meeting_audio(meeting)
    transcript_path.write_text(transcript, encoding="utf-8")

    minutes = build_minutes_from_transcript(meeting, transcript)
...
def transcribe_meeting_audio(meeting):
...
def build_minutes_from_transcript(meeting, transcript):
    ...
    llm_minutes = generate_minutes_with_ollama(meeting, date_text, transcript_body)
    if llm_minutes:
        return llm_minutes
    ...
    return {
        ...
        "actionItems": [
            {
                "task": "Install Ollama and pull the qwen2.5:3b model.",
                "owner": "User",
                "deadline": "Before final minutes generation",
                "notes": "Run: ollama pull qwen2.5:3b"
            }
        ],
        "risks": [
...
def generate_minutes_with_ollama(meeting, date_text, transcript):
